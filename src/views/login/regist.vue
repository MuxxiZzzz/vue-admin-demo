<template>
  <div class="login-container">
    <el-form ref="registForm" :model="loginForm" :rules="loginRules" class="login-form" auto-complete="on" label-position="left">
      ...
      <div style="display: flex">
        ...
        <el-button class="el-icon-back" :loading="loading" type="primary" style="width:15%;margin-bottom:30px;" @click.native.prevent="$emit('switchPage')" />
      </div>
  ...
  methods: {
    ...
    handleRegist() {
      this.$refs.registForm.validate(valid => {
        if (valid) {
          this.loading = true
          const { username, userid, useremail, password } = this.loginForm
          this.$store.dispatch(
            'user/regist',
            { name: username, surname: username, userName: userid, emailAddress: useremail, password: password },
            'outside'
          ).then(() => {
            this.loginForm.complex = this.loginForm.userid
